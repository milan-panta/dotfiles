...
function TopBar:init()
  ...
  local function maximized_command()
    if state.platform == "windows" then
      mp.command(
        state.border and (state.fullscreen and "set fullscreen no;cycle window-maximized" or "cycle window-maximized")
...
function TopBar:render()
  ...
  if main_title or state.has_playlist then
    ...
    if state.has_playlist then
      ...
      local opacity = get_point_to_rectangle_proximity(cursor, rect) <= 0 and 1 or config.opacity.playlist_position
      if opacity > 0 then
